...
export default function UserCard() {
  ...
  const {
    ...
    queryFn: () =>
      axios.post("http://localhost:3001/api/cards/allbyuserid", {userId: user.id}).then((res) => {
        return res.data;
      }),
  });
          ...
          {cards.length > 0
            ? cards.map((card) => (
                ...
                >
                  <p>
                    {card.name} **** **** ****{" "}
                    {card.number.slice(card.number.length - 4)} {card.month}/
                    {card.year}
                  </p>
                  <div className="flex items-center gap-x-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        axios
                          .post("http://localhost:3001/api/cards/delete", card)
                          .then((res) => {
                            if (res.status === 200) {
